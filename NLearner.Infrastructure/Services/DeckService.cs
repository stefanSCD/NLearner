...
namespace NLearner.Infrastructure.Service
{
    public class DeckService : IDeckService
    {
        ...
        {
            await using var _context = await _contextFactory.CreateDbContextAsync();
            var (card, project) = await GetOwnedCardAsync(_context, cardId, userId);
        ...
        public async Task<Deck?> GetDeckAsync(Guid deckId, string userId)
        {
            await using var _context = await _contextFactory.CreateDbContextAsync();
            var deck = await _context.Decks.FindAsync(deckId);
            if (deck is null) return null;
        ...
        public async Task<List<Card>> GetDueCardsAsync(Guid deckId, string userId)
        {
            var deck = await GetDeckWithCardsViewModelAsync(deckId, userId);
            if(deck == null)
        ...
        {
            await using var _context = await _contextFactory.CreateDbContextAsync();
            var (card, project) = await GetOwnedCardAsync(_context, dto.Id, userId);
            if(card is null || project is null)
                return null;
        ...
        public async Task UpdateCardReviewAsync(Guid cardId, ReviewQuality quality, string userId)
        {
            await using var _context = await _contextFactory.CreateDbContextAsync();
            var (card, project) = await GetOwnedCardAsync(_context, cardId, userId);
            if (card is null || project is null)
            ...
            else
            {
                ...
                else card.IntervalDays = (int)Math.Round(card.IntervalDays * card.EaseFactor);

                card.EaseFactor += (0.1f - (5 - (int)quality) * (0.08f + (5 - (int)quality) * 0.02f));
        ...
        private async Task<(Card? card, Project? project)> GetOwnedCardAsync(AppDbContext context, Guid cardId ,string userId)
        {
